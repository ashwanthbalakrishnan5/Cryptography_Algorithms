...
def pad(x: str, block: int) -> str:
    length = len(x)
    x = x + "1"
    while (len(x) + block // 8) % block != 0:
        x = x + "0"
    x = x + fill(bin(length)[2:], block // 8)
    # priclearnt(x)
    return x
...
def split_blocks(words: list, size: int) -> list:
    answer = [words[i : i + size] for i in range(0, len(words), size)]
    # print(answer)
    return answer
...
def sha512(message: str) -> str:
    ...
    blocks = split_blocks(
        [
            ...
            for word in split_words(
                pad(binary_val(message), K_BLOCK_LENGTH), K_WORD_LENGTH
            )
        ],
        16,
    )
    ...
    for block in blocks:
        ...
        for t in range(0, K_SCHEDULE_LENGTH):
            t1 = add(
                [us1_64(h[4]), choice(h[4], h[5], h[6]), h[7], block[t], k[t]],
                K_WORD_LENGTH,
            )
